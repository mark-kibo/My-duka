...
function SignUpAdmin() {
    ...
    function useEmailToken(emailToken) {
      return useQuery({
        queryKey: [""],
        queryFn: async () => {
    ...
    // Fetch token data using the custom hook
    const { data: tokenData, isError, error } = useEmailToken(emailtoken);
    ...
    if (isError) {
      return (
        <div className="alert alert-danger flex items-center justify-center h-screen">
          Token is invalid. Kindly request for another signup link.
          <Link to="/login">
          <button className="bg-sky-400 text white hover:bg-gray-200 transition duration-100 p-2 shadow-md rounded-md">
            go to login</button>
            </Link>
        </div>
      );
    ...
    if (!tokenData) {
      ...
      return <div className="flex items-center justify-center mx-auto h-screen">
    ...
    const storeOption = data?.map(store=>{
        return <option key={store.store_id} value={store.store_id}>{store.store_id}</option>
    })
  ...
  return (
    <>
    {error ? (
        <div className="alert alert-danger flex items-center justify-center mt-10">token is invalid  kindly request for another signup link</div>
    ): (
      ...
      >
        ...
        <div className="w-[100%] p-5 h-full  m-auto ">
          ...
          <div className="py-2 m-4 text-center">
            <h3 className="leading-4 uppercase font-bold ">MyDuka Signup</h3>
          </div>
          <form method="POST" onSubmit={handleSubmit}>
            <div className="mb-4">
              <label
                htmlFor="name"
                className="block mb-1 text-sm font-medium text-gray-900 dark:text-sky-900"
              >
                User Name
              </label>
              <input
